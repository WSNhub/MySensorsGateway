...
ethernetif_input(struct netif *netif)
{
  ...
  do {
    ...
    if((len = low_level_startinput(ethernetif->internals)) == 0)
      break;
    ...
    if (p != NULL) {
      ...
      for(q = p; q != NULL; q = q->next) {
        ...
        low_level_input(ethernetif->internals, q->payload, q->len);
      }
      //ethernetif->low_level_endinput(ethernetif->internals);
      low_level_endinput(ethernetif->internals);
      ...
      low_level_input_nomem(ethernetif->internals, len);
...
ethernetif_linkoutput(struct netif *netif, struct pbuf *p)
{
  ...
  if(!low_level_startoutput(ethernetif->internals))
	  return ERR_IF;
  ...
  for(q = p; q != NULL; q = q->next) {
    ...
    low_level_output(ethernetif->internals, q->payload, q->len);
  }

  //ethernetif->low_level_endoutput(ethernetif->internals, p->tot_len);
  low_level_endoutput(ethernetif->internals, p->tot_len);
...
ethernetif_init(struct netif *netif)
{
  ...
  low_level_init(ethernetif->internals, ethernetif->address, NULL);

...
// & to generate timestamp for data logging. MyDateTime Class is a modified
// version supporting day-of-week.

// Original MyDateTime Class and its utility code is by Jean-Claude Wippler at JeeLabs
...
////////////////////////////////////////////////////////////////////////////////
// utility code, some of this could be exposed in the MyDateTime API if needed

static const uint8_t daysInMonth [] PROGMEM = { 31,28,31,30,31,30,31,31,30,31,30,31 };
static byte DS3231_address;
...
////////////////////////////////////////////////////////////////////////////////
// MyDateTime implementation - ignores time zones and DST changes
// NOTE: also ignores leap seconds, see http://en.wikipedia.org/wiki/Leap_second

MyDateTime::MyDateTime (long t) {
    ss = t % 60;
    t /= 60;
    mm = t % 60;
    t /= 60;
...
MyDateTime::MyDateTime (uint16_t year, uint8_t month, uint8_t date, uint8_t hour, uint8_t min, uint8_t sec, uint8_t wd) {
    if (year >= 2000)
        year -= 2000;
    yOff = year;
    m = month;
...
//   MyDateTime now (__DATE__, __TIME__);
...
uint32_t MyDateTime::get() const {
    uint16_t days = date2days(yOff, m, d);
    return time2long(days, hh, mm, ss);
}

uint32_t MyDateTime::getEpoch() const
{
...
//set the time-date specified in MyDateTime format
...
MyDateTime Sodaq_DS3231::makeDateTime(unsigned long t)
{
  if (t < EPOCH_TIME_OFF)
    return MyDateTime(0);
  return MyDateTime(t - EPOCH_TIME_OFF);
}

// Set the RTC using timestamp (seconds since epoch)
void Sodaq_DS3231::setEpoch(uint32_t ts)
{
  setMyDateTime(makeDateTime(ts));
}
